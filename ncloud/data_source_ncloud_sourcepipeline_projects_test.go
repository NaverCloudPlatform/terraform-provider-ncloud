...
func TestAccDataSourceNcloudSourcePipelineProjects_classic_basic(t *testing.T) {
	dataName := "data.ncloud_sourcepipeline_projects.by_filter"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceNcloudSourcePipelineProjectsConfig("test-project", "description test"),
...
func testAccDataSourceNcloudSourcePipelineProjectsConfig(name, description string) string {
	return fmt.Sprintf(`
...
data "ncloud_sourcepipeline_projects" "by_filter" {
	filter {
		name = "id"
		values = [ncloud_pipeline_project.foo.id]
	}
}
`, name, description)
}
