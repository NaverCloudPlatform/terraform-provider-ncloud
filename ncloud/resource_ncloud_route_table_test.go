...
func testAccCheckRouteTableExists(n string, routeTable *vpc.RouteTable) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		instance, err := getRouteTableInstance(client, rs.Primary.ID)
		if err != nil {
			return err
		}
...
func testAccCheckRouteTableDisappears(instance *vpc.RouteTable) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		_, err := client.vpc.V2Api.DeleteRouteTable(reqParams)

		waitForNcloudRouteTableDeletion(client, *instance.RouteTableNo)
