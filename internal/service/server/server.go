...
func resourceNcloudServerRead(d *schema.ResourceData, meta interface{}) error {
	...
	// Populate the actual base volume size for KVM servers so state reflects reality.
	// This makes a matching config a no-op at plan and keeps expand-only accurate,
	// including when the attribute is newly adopted on an existing server.
	if ncloud.StringValue(r.HypervisorType) == "KVM" {
...
func validateBaseBlockStorageSizeDiff(_ context.Context, d *schema.ResourceDiff, meta interface{}) error {
	...
	// image was already validated at create and expand-only bounds the size, so skip the
	// lookup to avoid a plan-time API call on every diff. Fires only when the image is known.
	if d.Id() == "" && meta != nil {
