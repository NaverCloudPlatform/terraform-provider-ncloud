...
func TestAccDataSourceNcloudRedisProducts_vpc_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceNcloudRedisProductsConfig(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.ncloud_redis_products.all", "product_list.0.infra_resource_type", "VRDS"),
					resource.TestCheckResourceAttr("data.ncloud_redis_products.all", "product_list.0.product_type", "STAND"),
				),
			},
		},
	})
...
func testAccDataSourceNcloudRedisProductsConfig() string {
	return fmt.Sprintf(`
