...
* `regional` - (Optional) Whether to create a multi-zone (Regional) cluster. `true` | `false`(Default). When `true`, `zone` is not required. A Regional cluster can only use Kubernetes versions whose `isRegionalSupport` is `true` (see the `regional_support` argument of the `ncloud_nks_versions` data source). Multi-zone (Regional) clusters are only supported on the `public` site.
