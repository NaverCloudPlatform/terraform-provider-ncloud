...
* `base_block_storage_size` - (Optional) Size of the base (root) block storage in GB. Supported only on **KVM** servers created via `server_image_number`; it cannot be combined with `server_image_product_code` or `member_server_image_no`. Minimum `50` for Linux/Unix images and `100` for Windows images. The value can only be expanded, not shrunk. Changing it resizes the volume in place, which stops and restarts the server during the resize. Default: assigned by NAVER Cloud Platform.
