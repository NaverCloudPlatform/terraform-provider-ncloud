...
* `base_block_storage_volume_type` - (Optional, ForceNew) Volume type of the base (root) block storage, e.g. `CB1` or `CB2`. Accepted values are determined and validated by NAVER Cloud Platform, as the set may expand over time. Supported only on **KVM** servers created via `server_image_number`; it cannot be combined with `server_image_product_code` or `member_server_image_no`. This is the volume type, which is distinct from the read-only `base_block_storage_disk_type`. Default: assigned by NAVER Cloud Platform.
